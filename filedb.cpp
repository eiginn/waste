...
void C_FileDB::Search(char *ss, C_MessageSearchReply *repl, C_MessageQueueList *mqueuesend, T_Message *srcmessage, void (*gm)(T_Message *message, C_MessageQueueList *_this, C_Connection *cn))
{
  ...
  if (searchstring[0] == '/')
  {
    ...
    if (!searchstring[1] || (!strstr(searchstring+1,"/") && searchstring[strlen(searchstring)-1]=='*')) // respond with yourself
    {
      ...
      repl->add_item(-1,machinename,(char*)USER_STRING,m_database_used, GetNumKB(), GetLatestTime());
        ...
        if (!searchstring[0] || (!strnicmp(searchstring,buf,sslen) && (is_wc || buf[sslen]==0 || buf[sslen]=='/')))
        {
          ...
          if (stricmp(++f,last))
          {
            ...
            if (is_dir)
            {
              ...
              dbsize_errcnt += m_database[x].length_low & 0x3FF;
              dbsize_errcnt &= 0x3FF;
              poosize=(m_database[x].length_high * 4096*1024)+(m_database[x].length_low>>10) + ((dbsize_errcnt < o));
            }
          ...
          else
          {
            ...
            repl->addlastsize(1,(m_database[x].length_high * 4096*1024)+(m_database[x].length_low>>10) + ((dbsize_errcnt < o)),m_database[x].file_time); // if dbsize_errcnt wraps, then we should tack another mb on there
          }
        }
      }
    }
...
#endif
            if (stricmp(p,"PADLOCKSLSTATE") && in_list(m_ext_list,p))
            {
              if (s.dir_index==-1)
              {
                alloc_dir_index();
             ...
             m_database[m_database_used].length_high=0; // fucko64 for bsd
              ...
              m_database_xbytes+=m_database[m_database_used].length_low&0x3FF;
 	      // If m_database_xbytes > 1mb, add 1mb to m_database_mb
              if (m_database_xbytes > 0x3FF)
	      {
                m_database_xbytes &= 0x3FF;
	            m_database_kb ++;
              }

	      // I don't know what's the means of these codes :( -- lora.
              // The m_database_xbytes will never less than 0 !
              m_database_xbytes &= 0x3FF;
              if (m_database_xbytes < 0) 
	      {
	         m_database_kb++;
	
	      }

              m_database_kb+=(m_database[m_database_used].length_high*4096*1024) + (m_database[m_database_used].length_low>>10);
